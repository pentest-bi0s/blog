...
description: "Everything starts from a dot and builds up to two. Uniting them is like a kiss in the dark from a stranger. Made up horrors to help you cope with the real ones, join us to take a bite at the two-sentence horror stories on our very own TwoDots Horror™ blog."
...
![image1.png](/assets/images/twodotshorror/image1.png)
	...
	return db.getUser(req.data.username)
		.then(user => {
			...
			if(content){
				...
				if(twoDots == null || twoDots.length != 2){ // <--- Checking for two periods
				...
				return db.addPost(user.username, content)
					.then(() => {
						bot.purgeData(db); // <--- This is where the bot plays the role
...
![image2.png](/assets/images/twodotshorror/image2.png)
...
![image3.png](/assets/images/twodotshorror/image3.png)

 Interesting… , a CSP violation error. So what is a CSP ? 

![csp.png](https://prod-files-secure.s3.us-west-2.amazonaws.com/250b82a5-4a72-4378-9c99-b1b26f5c1edb/ad1b38b8-b443-4f1a-bc63-8c206032aa12/csp.png)
...
```html
<img class="nes-avatar is-rounded profile-avatar" src="/api/avatar/{{ user.username }}?t={{ range(1111, 9999)|random }}" /><br />
```
...
![flag.png](/assets/images/twodotshorror/image4.png)
